    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float4 cr = _Color*.8*f2*col*y;
                float3 zz = hsv2rgb(float3(i.wPos.x*.7,1,1));
                float4 rain = float4(zz.x,zz.y,zz.z,1);
                return length(_Color.rgb)>.01?cr:rain;
            }
            ENDCG
        }
